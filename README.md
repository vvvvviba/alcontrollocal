...
### 配置文件说明（准备发布到 GitHub 时建议阅读）

- 运行时配置会保存到 `backend/config.json`，包含 API Key 等敏感信息，**不要提交到 GitHub**。
- 仓库提供了示例文件：
  - `backend/.env.example`（环境变量示例）
  - `backend/config.example.json`（配置 JSON 示例）
- 如果你想用 `.env` 配置：将 `backend/.env.example` 复制为 `backend/.env` 再填写；也可以直接在网页设置里配置（推荐）。
...
**Q: 修改共享文件夹路径后为什么没有生效？**
A: 共享服务进程启动后会固定使用当时的共享目录。修改路径后，如果共享服务正在运行，保存时会提示是否立即重启；或手动执行 `stop share` 再 `start share`。
